...
exports.getMyPage = async (req, res) => {
    ...
    try {
        ...
        if (req.xhr || req.headers.accept.indexOf('json') > -1) {
            return res.status(200).json({ reviews: reviewsData.reviews, pagination: reviewsData });
        } else {
            return res.status(200).render('mypage', { member, reviews: reviewsData.reviews, pagination: reviewsData });
        }
    } catch (error) {
